...
RequestHandler::RequestHandler(Config* config) : _config(config) {
...
void	RequestHandler::readfile(const std::string &path) {
    ...
    Cgi	cgi(_uri, "/Users/wtaylor/Documents/School10.5/projects/webserv/cgi_tester", _method); //get script location from config

    if (cgi.isCgi(fileExеtention)) {
        body = cgi.processCgi(path);
        if (body.size())
            status_code = CGICODE;
    }
    else {
...
//RequestHandler::RequestHandler(IConfig &conf) : _conf(conf) {
//	_index.push_back("resources/html_data/index");
//	_index.push_back("resources/html_data/index.html");
//}
...
//RequestHandler::RequestHandler(Request &request, IConfig *conf) :
...
void RequestHandler::doPost(WebClient* client) {
...
void RequestHandler::doPut(WebClient* client) {

}

void RequestHandler::doDelete(WebClient* client) {
